...
class DbOperations {
            ...
            if ($username != "") {
                if ($email != "") {
                    if ($password != "") {
                        ...
                        $stmt->bind_param("ssssss", $username, $usernamelower, $email, $password, $date, $profile_pic);
                        if ($stmt->execute()) {
                            $stmt->close();
    ...
    public function getUserByUsername($username) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            // Create a while loop
            while ($stmt->fetch()) {
                $userArray = array('id' => $id, 'nickname' => $nickname, 'username' => $username2, 'email' => $email, 'profile_pic' => $profile_pic, 'users_followed' => $users_followed, 'games_followed' => $games_followed, 'users_friends' => $friend_array);
                return $userArray;
            }
        } else {
            return false;
